            ...
            for _, row in df.iterrows():
                ...
                for st in (1, 2, 3, 4):
                    ...
                    if a1_trim.size > 350:
                        ...
                        slopes = self.pointwise_slope(ap_trim)
                        mask = slopes > 0.1
                        runs = self.find_true_runs(mask)
                        rising_segs = [(s, e) for (s, e) in runs if (e - s + 1) >= 5]



                        s_a1 = seg_slope(a1_trim[:rising_segs[0][1]+1]) if a1_trim.size >= rising_segs[0][1]+1 else []
                        s_a2 = seg_slope(a2_trim[:rising_segs[0][1]+1]) if a2_trim.size >= rising_segs[0][1]+1 else []
                        station_saved[device_str][st][gaiban_str] = {
                            ...
                            "slope_arr1_rising_1_max": max(s_a1),
                            "slope_arr2_rising_1_max": max(s_a2)
                        }
...
class RuKeTrainServiceV1(BaseService):
    def __init__(self, db_client=None):
        self.db_client = db_client
        self._ready = False
