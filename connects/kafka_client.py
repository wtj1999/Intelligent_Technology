...
class KafkaClient:
    def __init__(self, kafka_params: dict = kafka_params):
        self.bootstrap_servers = kafka_params["bootstrap_servers"].split(",") \
            if isinstance(kafka_params["bootstrap_servers"], str) else kafka_params["bootstrap_servers"]
        self.topic = kafka_params["output_topic"]
        self._producer = None
